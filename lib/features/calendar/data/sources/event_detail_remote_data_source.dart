...
abstract class EventDetailRemoteDataSource {
  Future<EventModel> getEventDetail({required int eventId});
}

class EventDetailRemoteDataSourceImpl implements EventDetailRemoteDataSource {
  final DioClient _client;

  EventDetailRemoteDataSourceImpl(this._client);

  @override
  Future<EventModel> getEventDetail({required int eventId}) async {
    try {
      final response = await _client.get(ApiUrls.eventDetail(eventId));
